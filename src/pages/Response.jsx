      ...
      <div className="flex flex-col gap-6">
        {!orders.length ? (
          ...
          orders.map((order) => (
            <div
              key={order._id}
              className="rounded-lg shadow bg-white p-6 flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-6 flex-1">
                <div>
                  ...
                  <p className="text-sm">Approval Status: <span className="font-medium">{order.approval}</span></p>
                  <p className="text-sm">Delivery Status: <span className="font-medium">{order.available}</span></p>
                </div>
                <div className="sm:ml-auto">
                  <p className="text-sm text-gray-500">Order Created: {order.requiredDate}</p>
                  <p className="text-sm text-gray-500">Delivered: {order.deliveredDate}</p>
                </div>
              </div>
            </div>
          ))
